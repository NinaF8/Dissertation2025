...
var jsPsych = initJsPsych({
 ...
 /*
  var waiting_room = {
    ...
    stimulus: '<p style="font-size: 48px;"> Waiting for partner to join... </p>',
    choices: "NO_KEYS",
  ...
  */   
  ...
  var waiting_room = {
    ...
    stimulus: function () {
      return `
        <div id='processing-message'><p>Connecting to server...</p></div>
    ...
    on_load: function () {
      ...
      // Set total duration and step increment
      var duration = random_wait(); // Total duration for this trial
      var increment = 100 / (duration / 50); // Progress bar increment per step
      var width = 0;
  
      // Update messages at specific intervals
      setTimeout(() => {
        message.innerHTML = "<p>Loading ChatGPT interface...</p>";
      }, duration * 0.4); // Change message after 40% of the time
  
      setTimeout(() => {
        message.innerHTML = "<p>AI partner loaded.</p>";
      }, duration * 0.8); // Change message after 80% of the time
  
      // Animate the progress bar
      var interval = setInterval(function () {
        width += increment;
        bar.style.width = Math.min(width, 100) + "%";
        if (width >= 100) clearInterval(interval); // Stop animation when bar is full
      }, 50);
    },
  };
  /******************************************************************************/
 ...
 */
    ...
    stimulus: function () {
      ...
      return `
        <div id="processing-container">
          <p>Processing<span id="dots"></span></p>
        </div>`;
    },
    prompt: "<p><em>Click on the picture your partner described</em></p>",
    choices: shuffled_image_choices,
    ...
    var describe_image = {
       ...
       */
       ...
       var waiting_for_partner = {
        ...
        stimulus: function () {
          return `
            <div id='processing-message'><p>AI is processing your response...</p></div>
            <div style='width: 100%; background-color: lightgray;'>
              <div id='progress-bar' style='width: 0%; height: 30px; background-color: green;'></div>
            </div>`;
        },
        ...
        on_load: function () {
          ...
          // Update messages at specific intervals
          setTimeout(() => {
            message.innerHTML = "<p>Making picture selection...</p>";
          }, duration * 0.4); // Change message after 40% of the time
      
          setTimeout(() => {
            message.innerHTML = "<p>Picture selected.</p>";
          }, duration * 0.8); // Change message after 80% of the time
  ...
  */
  var write_headers = {
    ...
    func: function () {
      var this_participant_filename = "aicondition_" + participant_id + ".csv";
      save_data(
  ...
  */
  ...
  var consent_screen = {
    ...
    stimulus:
      "<div style='margin: 0 auto; max-width: 800px;'>\
      <h3>Welcome to our ChatGPT-powered experiment.</h3> \
    <p style='text-align:left'>Thank you for choosing to take part in our study! The session should take 15-20 minutes to complete.</p>\
    <p style='text-align:left'>Please ensure you are running the experiment on Google Chrome for the best experience. <u>Please ensure you are using a laptop.</u></p>\
    <p style='text-align:left'>This is a partner-based experiment. Your partner is an AI.\
    You will be playing a communication game - the text you see will be generated by a computer, not a person, and we will pass your descriptions to that same AI system.\
  ...
  var pre_interaction_instructions = {
    ...
    Ensure you are connected to the internet for the ChatGPT API to function effectively. This system will simulate an AI-powered interaction in real-time.\
    It can be a bit unstable, so please notify the experimenter if you experience any issues.</p>\
    ...
    your partner <b><u>using the given word.</u></b> \
